...
public class TheInternalInventory implements IInventory {
	...
	public ItemStack getStackInSlot(final int slotIndex) {
		if (slotIndex >= this.slots.length || slotIndex < 0) {
			DireLog.warning("Index out of range: %d", slotIndex);
			return null;
		}
	...
	/**
	 * Increases the stack size of the specified slot, and marks the inventory as
	 * dirty.
	 *
	 * @param slotIndex
	 * @param amount
	...
	public boolean isUseableByPlayer(final EntityPlayer player, final TileEntity tile) {
		return player != null && tile != null && canPlayerInteractWith(player, tile);
	}

	private static final boolean canPlayerInteractWith(final EntityPlayer player, final TileEntity tile) {
