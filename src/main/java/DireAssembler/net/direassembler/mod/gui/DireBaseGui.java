...
import DireAssembler.net.direassembler.mod.containers.ContainerWithNetworkTool;
import DireAssembler.net.direassembler.mod.gui.buttons.DireGuiButtonBase;
...
public abstract  class DireBaseGui extends GuiContainer implements IWidgetHost{
	...
	public DireBaseGui( final Container container )
	{
		super( container );
	}
	...
	private final boolean addTooltipFromButtons(final int mouseX, final int mouseY) {
		...
		for (Object obj : this.buttonList) {
			// Is it a base button?
			if (obj instanceof DireGuiButtonBase) {
				// Cast
				DireGuiButtonBase currentButton = (DireGuiButtonBase) obj;
	...
	private final boolean isPointWithinSlot(final Slot slot, final int x, final int y) {
		return DireGuiHelper.INSTANCE.isPointInGuiRegion(slot.yDisplayPosition, slot.xDisplayPosition, 16, 16, x, y,
	...
	private final boolean nonLeftClickHandler_Buttons(final int mouseX, final int mouseY, final int mouseButton) {
		if (mouseButton != DireGuiHelper.MOUSE_BUTTON_LEFT) {
			// Mouse over button?
		...
		for (int i = 0; i < this.inventorySlots.inventorySlots.size(); i++) {
			...
			if ((slot instanceof SlotNetworkTool) || (slot.inventory instanceof UpgradeInventory)) {
				...
				this.drawTexturedModalRect(this.guiLeft + slot.xDisplayPosition, this.guiTop + slot.yDisplayPosition,
						DireBaseGui.upgradeU, DireBaseGui.upgradeV, DireBaseGui.upgradeWidth, DireBaseGui.upgradeHeight);
			}
		}
	}
	...
	protected void mouseClicked(final int mouseX, final int mouseY, final int mouseButton) {
		...
		if (this.inventorySlots instanceof ContainerWithNetworkTool) {
			...
			if (((ContainerWithNetworkTool) this.inventorySlots).hasNetworkTool()) {
				...
				if ((slot != null) && (slot.getStack() != null) && (slot.getStack()
	 ...
	 * Note: Do not override, use {@link #onButtonClicked(GuiButton, int)
	 * onButtonClicked} instead.
	 *
	 * @see #onButtonClicked(GuiButton, int )
	 */
	@Override
	public final void actionPerformed(final GuiButton button) {
		this.onButtonClicked(button, DireGuiHelper.MOUSE_BUTTON_LEFT);
	}
